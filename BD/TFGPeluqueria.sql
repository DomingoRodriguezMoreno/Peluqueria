...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 06-05-2025 a las 08:04:19
...
(40, 1, '2025-05-08', '09:00:00', '13:00:00', 'reservada', 240, 1.00);
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `asignar_empleado_disponible` BEFORE INSERT ON `citas_servicios` FOR EACH ROW BEGIN
    DECLARE fecha_cita DATE;
    DECLARE hora_inicio TIME;
    ...
    -- Calcular hora_fin para este servicio
    SET hora_fin_servicio = ADDTIME(hora_inicio, SEC_TO_TIME(duracion_servicio * 60));

    -- Obtener el rol requerido para el servicio
    ...
    -- Si no hay empleados, cancelar inserción
...
--
ALTER TABLE `citas`
  MODIFY `id_cita` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=42;
